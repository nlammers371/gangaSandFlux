...
rng(421);
while sample_counter <= total_samples && ~all(sampled_flags)
    ...
    ranked_repo_id_list = repo_ids(repo_rank_list);
    primary_flag = false(size(ranked_repo_id_list));
    primary_flag(1:min([n_repos_per_region length(primary_flag)])) = true;
    
    % add to structure
    for i = 1:length(primary_flag)
        sample_struct(sample_counter).region_name = region_id_index{region_id};
        sample_struct(sample_counter).date_1 = dateStringCell{ranked_repo_id_list(i)};
        sample_struct(sample_counter).date_2 = datestr(date_id2,'mmddyyyy');
        sample_struct(sample_counter).repo_name_raw = NameIndexRaw1(ranked_repo_id_list(i));
        sample_struct(sample_counter).repo_name_clean = NameIndexClean(ranked_repo_id_list(i));
        sample_struct(sample_counter).primary_flag = primary_flag(i); 
        
        sample_counter = sample_counter + 1*primary_flag(i);
    end
end  
